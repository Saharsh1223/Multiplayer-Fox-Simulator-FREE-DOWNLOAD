...
namespace MalbersAnimations.Controller
{
    public abstract class State : ScriptableObject
    {
       ...
       // [Space]
        ...
        [Tooltip("Amount of time need to allow  this state to exit")]
       ...
       // [Space]
        ...
        public bool CanBeActivated
        {
            get
            {
                ...
                if (OnEnterCoolDown) return false;           //Check Exit Cooldown
        ...
        internal float CurrentExitTime { get; private set; }

        /// <summary>Current Time the state was Activated</summary>
        internal float CurrentEnterTime { get; private set; }
        ...
        internal void CheckCanExit()
        {
            ...
            {
                if (/*ExitCooldown == 0 ||*/ MTools.ElapsedTime(CurrentEnterTime, ExitCooldown))
                {
                    if (ExitOnMain)
                    {
    ...
    {
        SerializedProperty
           ID, Input, Priority, General, TryLoop, EnterTag, ExitTag, ExitFrame, ExitOnMain, ExitCooldown, EnterCooldown,
            CanStrafe, MovementStrafe, IdleStrafe, debug,
